...
public record TransactionResponseDTO(
        ...
        @Schema(description = "Valor da transação", example = "42.50")
        BigDecimal amount,

        @Schema(description = "Descrição do lançamento", example = "Almoço no restaurante")
        String description,

        @Schema(description = "Tipo da transação")
        TipoTransacao transactionType,

        @Schema(description = "Período correspondente (ano-mês-01)", example = "2026-06-01")
        LocalDate period,

        @Schema(description = "Indica se a transação já foi efetivada", example = "true")
        Boolean isSettled
) {
}